...
public class PathAnimator2 extends AbstractPathAnimator2 {

    private final AtomicInteger mCounter = new AtomicInteger(0);
    private Handler mHandler;

    public PathAnimator2(AbstractPathAnimator2.Config config) {
        super(config);
        mHandler = new Handler(Looper.getMainLooper());
    }

    @Override
    public void start(final View child, final ViewGroup parent, boolean isLeftToRight) {
        parent.addView(child, new ViewGroup.LayoutParams(mConfig.heartWidth, mConfig.heartHeight));
        PathAnimator2.FloatAnimation2 anim = new PathAnimator2.FloatAnimation2(createPath(mCounter, parent, 2, isLeftToRight), randomRotation(), parent, child);
        ...
        anim.setInterpolator(new LinearInterpolator());
        //anim.setInterpolator(new JellyInterpolator());
        child.startAnimation(anim);
    }

    static class FloatAnimation2 extends Animation {
        private PathMeasure mPm;
        private View mView;
        private float mDistance;
        private float mRotation;

        public FloatAnimation2(Path path, float rotation, View parent, View child) {
